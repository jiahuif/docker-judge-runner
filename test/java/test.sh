...
function run_judge {
    ...
    # run with docker
    docker run --read-only --rm --net none -v `pwd`/build/target:/target:ro -v `pwd`/build/input.txt:/tmp/stdin.txt -v `pwd`/build/stdout.txt:/tmp/stdout.txt -v `pwd`/build/stderr.txt:/tmp/stderr.txt indeedplusplus/judge-runner:java || exit 1
